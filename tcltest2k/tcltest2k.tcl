...
##### Export the public tcltest procs; several categories
    ...
    #cow
    namespace export removeTestConstraint
    #cow 
...
proc tcltest::removeTestConstraint {constraint} {
    variable testConstraints
    variable Option
    DebugPuts 3 "entering removeTestConstraint $constraint"
    if {[catch {unset testConstraints($constraint)} msg]} {
	return -code error $msg
    }
}
#cow
...
proc tcltest::test {name description args} {
    ...
    #cow
    # Run any pending setup things
    variable setupRequests
    set setupError no
    for {set i 0} {$i < [llength $setupRequests]} {incr i} {
	set setupRequest [lindex $setupRequests $i]
	if {[dict get $setupRequest status] eq "done"} {
	    continue
	}
	if {[dict get $setupRequest status] eq "error"} {
	    lappend constraints setup_[dict get $setupRequest name]
	    break
	}
	lset setupRequests $i [dict set setupRequest status done]
	if {[dict get $setupRequest script] eq ""} {
	    continue
	}
	if {[IsVerbose setup]} {
	    puts [outputChannel] "---- setup [dict get $setupRequest name]"
	}
	set code [catch {uplevel 1 [dict get $setupRequest script]} setupMsg]
	if {$code == 1} {
	    set errorInfo(setups) $::errorInfo
	    set errorCode(setups) $::errorCode
	    set setupError yes
	    break
	}
    }
    if {$setupError} {
	lset setupRequests $i [dict set setupRequest status error]
	set setupConstraint setup_[dict get $setupRequest name]
	puts [outputChannel] "---- Test setup\
		failed:\n$setupMsg"
	if {[info exists errorInfo(setups)]} {
	    puts [outputChannel] "---- errorInfo(setups): $errorInfo(setups)"
	    puts [outputChannel] "---- errorCode(setups): $errorCode(setups)"
	}
	puts [outputChannel] "---- Adding constraint: $setupConstraint"
	testConstraint $setupConstraint 0
	lappend constraints $setupConstraint
    }
    #cow

    #cow
    set printdescr [IsVerbose descr]
    #cow

    #cow
    set descr [expr {$printdescr ? [string cat "  " [string trim $description]] : ""}]
    if {[Skipped $name $constraints $descr]} {
	incr testLevel -1
	return
    }
    #cow
	...
	if {[IsVerbose start]} {
	    #cow
	    puts [outputChannel] "---- $name start$descr"
	    if {$printdescr} { set printdescr 0; set descr "" }
	    #cow
	    flush [outputChannel]
	}
	...
	if {$testLevel == 1} {
	    ...
	    if {[IsVerbose pass]} {
		#cow
		puts [outputChannel] "++++ $name PASSED$descr"
		#cow
	    }
	}
	incr testLevel -1
...
# Skipped --
#
# Given a test name and its constraints, returns a boolean indicating
...
#cow
proc tcltest::Skipped {name constraints {descr {}}} {
#cow
	...
	if {!$doTest} {
	    if {[IsVerbose skip]} {
		#cow
		puts [outputChannel] "++++ $name SKIPPED: $constraints$descr"
		#cow
	    }
...
#cow
namespace eval tcltest {
    variable setupRequests [list]
    variable setupCounter -1
    variable cleanupCounter -1
    namespace export setup cleanup
}

proc tcltest::setup {nameorscript {script {}}} {
    variable setupRequests
    variable setupCounter
    if {$script eq {}} {
	set script $nameorscript
	set nameorscript [incr setupCounter]
    }
    lappend setupRequests [dict create name $nameorscript script $script status ready]
}

proc tcltest::cleanup {nameorscript {script {}}} {
    variable setupRequests
    variable cleanupCounter
    if {[llength $setupRequests] == 0} {
	return
    }
    if {$script eq {}} {
	set script $nameorscript
	set nameorscript [incr cleanupCounter]
    }
    set setupRequest [lindex $setupRequests end]
    set setupRequests [lrange $setupRequests 0 end-1]
    if {[dict get $setupRequest status] ne "done"} {
	return
    }
    if {[IsVerbose cleanup]} {
	puts [outputChannel] "---- cleanup $nameorscript ([dict get $setupRequest name])"
    }
    set code [catch {uplevel 1 $script} cleanupMsg]
    if {$code == 0} {
	return
    }
    set errorInfo(cleanups) $::errorInfo
    set errorCode(cleanups) $::errorCode
    puts [outputChannel] "---- Test cleanup\
        failed:\n$cleanupMsg"
    if {[info exists errorInfo(cleanups)]} {
        puts [outputChannel] "---- errorInfo(cleanups): $errorInfo(cleanups)"
        puts [outputChannel] "---- errorCode(cleanups): $errorCode(cleanups)"
    }
}
#cow
